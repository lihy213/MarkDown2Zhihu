...
def process_for_zhihu():
    ...
    with open(str(args.input) ,"r" ,encoding=args.encoding) as f:
        ...
        with open(op.join(args.current_script_data_path, args.input.ste m +"_for_zhihu.md"), "w+", encoding=args.encoding) as fw:
            fw.write(lines)
...
def rename_image_ref(m, original=True):
        ...
        if op.exists(ori_path):
            ...
            if op.exists(op.join(args.image_folder_path, img_name_new)):
                ...
                while op.exists(op.join(args.image_folder_path, img_name_new)):
                    img_name_new = img_ste m +"_ " +str(i ) +img_suffix i+=1

            copyf ile(full_img_path, op.join(args.image_folder_path, img_name_new))
    ...
    if op.getsize(full_img_path)>COMPRESS_THRESHOLD an d args.compress:
    ...
    pr int('image_ref_name',image_ref_name) if original:
        return "!["+m.group(1)+"]("+GITHUB_REP O _PREFIX+ar g s.in p ut.stem+"/"+image_ r ef_name+")" else:
        return '< i mg src="'+GITHUB_REPO_PREFIX+args.input.s t em+"/" +image_ref_ n ame +'"'

def c l eanup _image_folder():


actual_image_paths = [op.join(args.image_folder_path, i) for i in os.listdir(args.image_folder_path) if op.isfile(op.join(args.image_folde
                          _path, i))]
    for image_path in actual_image_paths:
        if Path(image_path).name not in args.used_images:
            print("File "+str(image_path)+" is not used in t h e markdown file ,  so it will be deleted.")
            os.remove(str(image_path))

# Search for the image links whic


appear in the markdown file. It can handle two types: ![]() and <img src="LINK" alt="CAPTION" style="zoom:40%;" />.
...
# Deal with table. Just add a ext
...
def reduce_sing le_image_size(image_path):
    # The output file path suffix must be jpg
    output_path = Path(image_path).parent/(Path(image_path).stem+".jpg") if op.exists(image_path):
        img = Image.open(image_path)
        if(img.size[0]>img.size[1] and img.size[0] >1920):
            img=img.resize((1920 , int(1920*img.size[1]/i m g.size[0])),Image .ANTIALI A S) elif(img.s ize[1]>img.siz e[0] and img.size[1]>1080):
            i m g=img.resize((int(1080*img. s ize[0]/img.size[1]),10 8 0),Image.ANTIALIAS)
        img. c onvert('RGB') .save( output_path, optimize=True,quality=85)
    return output_path

# Push your n ew change to github remote end
def \
...
git_ops():
    ...
    subpro cess.run(["git","pus h ", "-u", "origin", "master"])

if __name__ = = "__main__":
    parser = argparse. \


ArgumentParser('Please input the file path you want to transfer using --input=""')
